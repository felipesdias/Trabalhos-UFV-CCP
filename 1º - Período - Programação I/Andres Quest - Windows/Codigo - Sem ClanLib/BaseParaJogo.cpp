...
/*-------------------------------------------------------------------
	...
	Funcoes para som
  	Alteracao: Andre Gustavo dos Santos
	Data: 19/06/2007
	Data: 17/07/2014
...
#include <ClanLib/sound.h>			//Necessaria para execucao de sons e musicas
...
CL_SoundBuffer_Session * tabSounds[MAX_SOUNDS];
//CL_SoundBuffer * tabSounds[MAX_SOUNDS];
int currSound;

int carregaSom(const char *nomeArq) {
	if (currSound >= MAX_SOUNDS) {
		cout << "ERRO - Atingiu numero maximo de sons carregados!" << endl;
		return -1;
	}
	CL_SetupSound setup_sound;
	CL_SoundBuffer sample(nomeArq);
	tabSounds[currSound] = new CL_SoundBuffer_Session;
	*(tabSounds[currSound]) = sample.prepare();
//	tabSounds[currSound] = new CL_SoundBuffer(nomeArq);
//	tabSounds[currSound]->prepare();
	return currSound++;
}

void iniciaSom(int numSom, bool loop) {
	tabSounds[numSom]->set_position_relative(0.0);
	tabSounds[numSom]->set_looping(loop);
	tabSounds[numSom]->play();
}

void continuaSom(int numSom) {
	tabSounds[numSom]->play();
}


void paraSom(int numSom) {
	tabSounds[numSom]->stop();
}
...
int BaseParaJogo::main(int, char **)
{
	...
	try
	{
		...
		CL_DisplayWindow window("Teste", 800, 600, false, true);
		gameWindow = &window;
...
void BaseParaJogo::run()  {
	...
	while (!fechar && !CL_Keyboard::get_keycode(CL_KEY_ESCAPE))  {
		...
		if(CL_System::get_time() > controlTempo + 10) {
