...
int linha[8];   // Vetor que indica quantas peças (valor) podem ser jogadas em cada coluna (índice).
int numero_simulacoes;   // Máximo de simulações que podem ser feitas pelas heurísticas.
clock_t clock_inicial;   // Variável para marcar o tempo.
...
int monteCarlo(Jogada_t jogada_inicial);   // Heurística de Monte Carlo.
...
int main() {
    ...
    int simulacoes_computador_1, simulacoes_computador_2; //Define nume de simulações a ser utilizada na heuristica
    ...
    do { // Repete enquanto quer jogar
        ...
        for(int it=1; it<=42; it+=2) { 
            ...
            if(opcao == 1 || opcao == 4) { 
				//Lê uma coluna até que ela seja válida
                do { 
            ...
            else { //Se não, é um computador, logo ele joga
                
                Jogada_t jogada_aux(jogada); //Jogada auxiliar para não ter perda de valores
                jogada.setAlpha(INT_MIN); //Computador 1 deseja maximizar o alpha
                clock_inicial = clock(); //Inicializa clock
                tempo = double(clock()-clock_inicial)/CLOCKS_PER_SEC;//Começa contar o tempo
                ...
                for(int profundidade=1; tempo < tempo_limite_computador_1 && profundidade <= 43-jogada_aux.getNumero(); profundidade++) {
                    //Recebe melhor jogada, de acordo com as configurações estabelecidas
                    jogada_aux.setJ(0);
            ...
            if(opcao == 2 || opcao == 4) { //Análogo para a parte superior, com Jogador 2 e Computador 2
                do {
...
void inicializaVariaveis(int tabuleiro_aux[8][9], int linhas_aux[8], double &tempo_aux1, double &tempo_aux2, int opcao, string &nome_player_1, string &nome_player_2, int &heuristica_computador_1, int &heuristica_computador_2, bool &poda_alpha_beta_computador_1, bool &poda_alpha_beta_computador_2, int &simulacoes_computador_1, int &simulacoes_computador_2) {
    ...
    // Se opção for 2 ou 3, então computador 1 participa do jogo
    if(opcao == 2 || opcao == 3) {
    ...
    // Se opção for 3 ou 1, então computador 2 participa do jogo
    if(opcao == 3 || opcao == 1) {
    ...
    // Se opção for 1 ou 4, então jogador 1 participa do jogo
    if(opcao == 1 || opcao == 4) {
        do {
    ...
    // Se opção for 2 ou 4, então jogador 2 participa do jogo
    if(opcao == 2 || opcao == 4) {
        do {
    ...
    //Cerca matriz do tabuleiro e inicializa a posição das colunas
	for(int i=0; i<8; i++)
...
bool empate(int linhas_aux[8]) {
    for(int it=1; it<=7; it++)   // Para cada coluna
        if(linhas_aux[it] > 0)   //se existir alguma coluna que ainda não está cheia
            return false;   //ainda não empatou

    return true;   // Caso contrário é um empate.
}

int monteCarlo(Jogada_t jogada_inicial, int numero_simulacoes) {
    int numero_vitorias = 0;   // Variável que contabiliza o número de vitórias das simulações.
    ...
    for(int i=0; i<numero_simulacoes; i++) {    // Laço para realizar as simulações
		...
		for(int j=jogada_inicial.getNumero()+1; j<=42; j++) {    // Enquanto existirem jogadas válidas
            jogada.aleatoriaMonteCarlo(tabuleiro_aux, linhas_aux);   // Cria uma jogada aleatória
			...
			if(jogada.vitoria(tabuleiro_aux)) {   // Se a jogada for vitoriosa
				if(jogada_inicial.getJogador() == jogada.getJogador())   //e for realizada pelo jogador inicial
					numero_vitorias++;   //significa que a vitória é dele.
				else   // Caso contrário
					numero_vitorias--;   //a vitória é do oponente.
				break;   // Termina a simulação atual.
			}
    ...
    return numero_vitorias;   // Retorna o numero de vitórias menos o número de derrotas.
}

int vouTeGanhei(Jogada_t jogada_inicial, int numero_simulacoes) { //Análogo ao monteCarlo
    ...
    for(int i=0; i<numero_simulacoes; i++) {
		...
		for(int j=jogada_inicial.getNumero()+1; j<=42; j++) {
            jogada.aleatoriaVouTeGanhei(tabuleiro_aux, linhas_aux); //Diferença para o monteCarlo
...
Jogada_t minMax(Jogada_t jogada, int profundidade, double tempo_atual, double tempo_limite, int heuristica, int numero_simulacoes, bool poda_alpha_beta, int poda) {
    ...
    if(jogada.getJ() != 0) {   // "jogada.getJ()" só será 0 quando minMax for chamada da função main();
        if(jogada.vitoria(tabuleiro)) {   // Se a jogada analisada for uma vitória
            jogada.setAlpha(jogada.getJogador()*(INT_MAX-jogada.getNumero()));   //retorna um valor máximo para o jogador 1 ou mínimo para o jogador -1.
            return jogada;
        }

        if(profundidade == 0 || tempo_atual >= tempo_limite) {   // Se o aprofundamento iterativo chegou ao fim ou o tempo de busca esgotou
            if(heuristica == 0)
                jogada.setAlpha(jogada.getJogador()*monteCarlo(jogada, numero_simulacoes));   //retorna o valor heurístico para o nó.
            else
                jogada.setAlpha(jogada.getJogador()*vouTeGanhei(jogada, numero_simulacoes));   //retorna o valor heurístico (parte 3) para o nó.
            return jogada;
        }

        if(jogada.getNumero() == 42) {   // Se for a 42º jogada, significa que o jogo empatou.
            jogada.setAlpha(0);   // Retorna 0, que é pior que a vitória, mas melor que a derrota.
            return jogada;
        }
    }

    Jogada_t melhor_jogada;   // Objeto auxiliar para guardar a jogada com maior valor heurístico.

    jogada++;   // Inverte o jogador e incrementa o número da jogada, para calcular o valor de minMax para seus filhos.

    if(jogada.getJogador() == 1) {   // Caso esteja na vez do jogador 1, deseja-se receber o valor máximo
        melhor_jogada.setAlpha(INT_MIN);   // Inicializa-se o "alpha" da melhor jogada com menos infinito.

        for(int it=1; it<=7; it++) {   // Para cada coluna
            if(linha[it] > 0) {   //se ela não está cheia
    ...
    else {   // Funciona de modo análogo para o jogador -1, entretanto deseja-se minimizar o valor de "alpha".
    ...
    return melhor_jogada;   // Caso o tempo não tenha se esgotado retorna a melhor jogada encontrada.
}
